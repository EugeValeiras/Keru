...
La autorización (¿esta cuenta puede leer/registrar sobre este paciente?) la decide **un único** `PermissionEngine`; **ningún Manager decide permisos por su cuenta**. El engine no sabe de dónde salen los datos: depende de un **contrato** (`AuthorityProvider`, patrón *port*) que responde "¿qué rol tiene la cuenta en el vínculo?", "¿tiene asignación **vigente en `at`** (dentro de la ventana)?" y "¿tiene una **relación de servicio viva** (asignación aceptada sin cerrar)?".

- **Lectura clínica ≠ escritura clínica — dos alcances distintos para el cuidador (KER-57).** La **ESCRITURA** clínica (registrar/corregir vitales, medicación, novedades) se autoriza por la **VENTANA** de la asignación evaluada al **tiempo de medición** (`periodStart ≤ measuredAt ≤ periodEnd`): fuera de ventana, una llegada de un cuidador con relación de cuidado va a **cuarentena**, nunca se descarta (NFR-30). La **LECTURA** clínica (estado, historial, gráficos — UC-14/15) del cuidador se autoriza por la **VIDA del servicio aceptado**: cualquier asignación en estado `active` (aceptada y aún no cerrada a `historical`) con el paciente, **sin** exigir que el instante caiga dentro de la ventana. Racional: NFR-30 ata a la ventana el *acto de medir*, no el de *mirar*; el wizard de contratación exige inicio futuro, así que atar la lectura a la ventana dejaría al cuidador sin acceso hasta el día de inicio (callejón sin salida). Mínimos privilegios (§2.10) se preserva porque el barrido de vencidos (NFR-14) pasa la asignación a `historical` al cerrarse/vencer, cortando la lectura. El vínculo **familiar** (cualquier rol de vínculo) lee y escribe sin ventana, como siempre. Esta es la **única** fuente de la regla (§3.5): la affordance del cliente se alinea a ella, nunca la sortea.
...
- **UC-02 A3:** cambio de **credenciales** (certificaciones/especialidades) de un cuidador **aprobado** — ¿dispara re-verificación, y de qué tipo (solo la credencial nueva o todo el perfil)? Hasta decidirlo, un perfil aprobado no puede **editar** las existentes; solo edita disponibilidad, tarifas (efectivo-fechadas), zona y modalidades. **Acotado por ADR-0003:** **nombre y foto salieron de esta lista** — no son credenciales verificadas (la insignia de identidad NFR-19 es un artefacto separado); la identidad del cuidador se deriva de su cuenta y se edita libremente por `PATCH /accounts/me` (UC-23), sin re-verificación. **Acotado por KER-52 (certificaciones):** cada certificación es una **credencial verificada individualmente** — el cuidador puede **incorporar una nueva** del catálogo finito con su documento privado (nace pendiente/oculta y pasa por la aprobación por-cert del admin, UC-19); una vez **aprobada o rechazada** es **inmutable** (no se edita, coherente con esta decisión), y mientras está **pendiente** solo puede re-subir el documento. Sigue abierto **solo** el caso de **editar/re-verificar una certificación ya aprobada** (borrarla o reemplazarla): no implementado; el camino soportado es agregar una nueva.
