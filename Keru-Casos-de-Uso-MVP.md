...
- **Flujo principal:**
  ...
  3. Carga sus certificaciones (KER-52). **No es texto libre:** elige el tipo de un **catálogo finito** (título de enfermería, auxiliar, RCP, cuidado geriátrico, cuidados paliativos, primeros auxilios, demencia/Alzheimer…) — cada entrada del catálogo tiene una **insignia propia** (ícono + nombre visible). Por cada certificación completa **institución y año** y **adjunta el documento escaneado** (PDF o imagen) que la respalda. Ese documento es **privado**: se guarda en almacenamiento no público (no tiene URL pública) y **solo el administrador** puede descargarlo para verificarla (UC-19). Cada certificación **nace no verificada y oculta al público** (no aparece en el marketplace/ficha hasta que el admin la apruebe, UC-19).
...
- **Flujos alternativos / excepciones:**
  - A1. Certificaciones sin institución, año, **tipo fuera del catálogo** o **sin documento adjunto**: el sistema las rechaza (un tipo que no pertenece al catálogo finito → **400/422**; falta el adjunto privado → error de validación). El adjunto valida **tipo** (solo `application/pdf` o imagen) y **tamaño** (máx. 10 MB).
  - A4. **Agregar certificaciones (KER-52):** el cuidador puede **sumar** una certificación nueva del catálogo en cualquier momento (incluso con el perfil ya aprobado), eligiendo el tipo + adjuntando su documento privado. Es una operación **aditiva**: la certificación nueva **nace pendiente y oculta**, entra a la cola de revisión del admin (UC-19) y **no toca** las credenciales ya aprobadas — por eso no contradice §7 (que prohíbe **editar** una credencial aprobada, no incorporar una nueva sujeta a verificación). Mientras una certificación está **pendiente**, el cuidador puede **re-subir su documento** (corrección de una postulación en revisión); una vez **aprobada o rechazada** queda **inmutable** (§7).
...
- **Criterios de aceptación:**
  ...
  - [ ] **Catálogo finito de certificaciones (KER-52):** el tipo de cada certificación se elige de un **catálogo finito seedeado** (no texto libre); cada entrada del catálogo tiene su **insignia** (ícono + nombre). Un tipo **fuera del catálogo** se rechaza (400/422). Hay un endpoint para que la webapp liste el catálogo.
  - [ ] **Adjunto privado por certificación (KER-52):** cada certificación lleva un **documento** (PDF o imagen, tipo y tamaño validados) subido a almacenamiento **no público** (sin URL pública); solo el **administrador** lo descarga (UC-19); cada acceso queda **auditado**. La subida crea la certificación con `operationId` (idempotencia, NFR-34).
  - [ ] Una cuenta de cuidador nueva **no es visible en el marketplace** hasta ser verificada y aprobada por el administrador (UC-19).
  - [ ] El cuidador puede ver el estado de su cuenta (pendiente / aprobada / rechazada) y el estado **por-certificación** (pendiente / aprobada / rechazada).
  - [ ] Las certificaciones nacen en estado "no verificada" **y ocultas al público** hasta que el proceso interno las apruebe **una por una** (UC-19); una certificación no aprobada **no aparece** en el marketplace/ficha.
...
- **Flujos alternativos / excepciones:**
  - A1. **Invitado no registrado — registro guiado por la invitación (KER-67):** al confirmar, el sistema lo envía al registro para crear su usuario; al completarlo, el vínculo con el paciente se establece automáticamente (la invitación no se pierde durante el registro: viaja en el `returnUrl` de vuelta a la landing). El registro **por invitación** no arranca en blanco — lo pre-configura la invitación para evitar errores y edición indebida:
    - **Email prellenado y NO editable** (readonly): es el **objetivo** de la invitación y **debe coincidir** para confirmar (desafío de identidad, NFR-19); dejarlo editable permitiría registrar otro email que luego el confirm rechaza. El campo sigue **enviando su valor** (accesible AA: readonly, anunciado, no un `disabled` mudo).
    - **Tipo de usuario (rol de cuenta) autoseleccionado `family` y bloqueado:** el círculo se compone solo de cuentas `family` y **confirmar exige `family`** (A6/KER-50) — un `caregiver` jamás podría aceptar. Por eso el signup por invitación **no ofrece elegir rol de cuenta**: se fija `family`. *(«Tipo de usuario» aquí es el **rol de cuenta**, no el rol del vínculo.)*
    - **Rol del vínculo (`roleToGrant`: `manager`/`viewer`) — informativo, no editable por el invitado:** lo fijó **quien invitó** (flujo paso 1 / A7); el invitado no lo elige. El **preview** de la invitación lo **expone** (`roleToGrant`) para mostrarlo en contexto («te sumás como Gestor» / «como Solo ver»); el vínculo se crea con ese rol al confirmar (`consent-holder` no se otorga por invitación — nace al registrar, UC-01).
    - **Nombre editable con placeholder:** el nombre es la **identidad propia** de la cuenta del invitado (ADR-0003, fuente única) y quien invita **solo conoce el email** — no se prellena ni se bloquea (forzar un nombre ajeno impediría corregirlo). La invitación **no captura** un nombre del invitado.
    - **Signup normal (sin invitación):** sin cambios — email, tipo de usuario (`family`/`caregiver`, UC-04) y nombre **todos editables**. El branch lo decide la **presencia de la invitación** (el `returnUrl` apunta a `/invite/:token`).
  ...
  - A7. **Re-asignar el rol de un vínculo ya existente (post-invitación → círculo, UC-22):** los roles del círculo se fijan al **invitar** (este UC otorga `manager`/`viewer`; el `consent-holder` nace al **registrar** el paciente, UC-01). **Cambiar** el rol de un miembro **ya vinculado** (promover un `viewer` a `manager`/`consent-holder`, o degradar) no es una nueva invitación: es una operación de gestión del **círculo** especificada en **UC-22** (A3/A4). La ejecuta un `consent-holder`, autorizada por el `PermissionEngine`, **auditada**, y **nunca** puede dejar al paciente sin `consent-holder` (política de titularidad, UC-22 A4).
...
- **Criterios de aceptación:**
  ...
  - [ ] **Envío best-effort pero OBSERVABLE (KER-66):** un fallo de envío no invalida ni bloquea la invitación, pero **nunca se traga en silencio** — se loguea a **ERROR** con detalle accionable (tipo de email, destinatario **enmascarado** para no exponer PII de más, `name`/`message` del error y stack). Aplica igual a los emails de **reset** y **verificación** (UC-04 A4/A5). Nota de entrega (diagnóstico KER-66, con evidencia): en **dev** SES está **emulado por floci** (`SERVICES=ses`). Se comprobó con una sonda directa que floci **acepta el `SendEmail` aun sin identidad verificada, lo captura y lo retorna** por su endpoint de introspección `GET {AWS_ENDPOINT_URL}/_aws/ses` (devuelve `Source`, `Destination`, `Subject` y `Body` con el link correcto). Es decir: el email **no se pierde** en floci; simplemente **no se entrega a un buzón real** (es un emulador). Por eso "no llega" en dev es esperado: la verificación correcta es inspeccionar `GET http://localhost:4566/_aws/ses` (o ver la línea de log de éxito `Email enviado a …`), no revisar una casilla real. En **prod** la identidad remitente debe estar **verificada en SES** (el sandbox exige remitente —y en sandbox también destinatario— verificados). **Decisión KER-66:** los emails transaccionales mantienen el **envío directo** best-effort (postcondición del UC; §3.1 los trata como Utility transversal); **no** se rutean por el outbox de KER-33 — encolar con retry/DLQ excede el fix puntual y queda como **follow-up** (dependencia KER-33).
  ...
  - [ ] **Registro por invitación pre-configurado (KER-67):** en el signup **por invitación** el **email** viene prellenado y **no editable** (readonly, pero envía su valor y se anuncia como solo-lectura, AA); el **tipo de usuario / rol de cuenta** queda **autoseleccionado `family` y bloqueado** (confirmar exige `family`, A6); el **nombre** queda **editable** (identidad propia, ADR-0003) con placeholder. El **signup normal** (sin invitación) mantiene **todos los campos editables** (email, rol `family`/`caregiver`, nombre). El branch lo decide la presencia de la invitación (`returnUrl` = `/invite/:token`).
  - [ ] **El preview expone `roleToGrant` (KER-67):** `GET /invitations/:token` devuelve, además de `patientId`/`patientName`/`invitedEmail`/`expiresAt`/`valid`, el **rol del vínculo a otorgar** (`roleToGrant`: `manager`/`viewer`) para mostrarlo informativo en la landing/registro; el contrato queda **tipado** (DTO) y `openapi.json`/`schema.d.ts` regenerados.
...
- **Flujos alternativos / excepciones:**
  ...
  - A5. **Verificación de email del self-signup (que el email sea realmente del titular):** una cuenta creada por auto-registro desde la plataforma (`POST /auth/signup`, email+password+rol) debe **probar que el email le pertenece**. Aplica **solo al self-signup**; las cuentas por **invitación** (UC-03) ya prueban el email al aceptar el link (NFR-19), por un camino propio.
    ...
    2. **Confirmación (`confirm`) según la sesión del browser (KER-63):** el link del email abre una pantalla que, **antes de consumir el token**, hace un **`peek` sin efecto** (`POST /auth/email-verification/confirm` **no** se llama a ciegas): `POST /auth/email-verification/peek` con el token devuelve solo el **email destino** del token si es válido (pendiente, no expirado) — o **410** si es inválido/expirado/reusado, sin consumirlo ni distinguir cuál de las tres. Con el email destino, la pantalla **ramifica por la sesión activa**:
       - **Sin sesión** → llama `confirm`: marca `emailVerified=true`, **consume el token** (single-use), audita el uso y devuelve una **sesión nueva** (auto-login) ya verificada + **feedback de éxito**.
       - **Sesión = la misma cuenta del token** (email de la sesión == email destino) → llama `confirm` igual: verifica y **renueva la sesión SIN pedir credenciales**, con feedback de éxito ("Tu email quedó verificado").
       - **Sesión = otra cuenta** → **no cambia de identidad en silencio**: **no** llama `confirm`, **cierra la sesión** actual (`clear`) y manda a **login pidiendo email+password** (email destino **prefilleado**, con `returnUrl` de vuelta a la verificación) y un **aviso** del porqué ("Este enlace es de otra cuenta; iniciá sesión con esa cuenta"). Al loguearse con la cuenta correcta, el `returnUrl` reabre la verificación —ahora "misma cuenta"— y **confirma**. Cambiar de identidad por seguir un link sin credenciales está **prohibido** (seguridad; el `peek` es el mecanismo para conocer el destino sin gastar el token ni switchear).
       El `confirm` sigue siendo **single-use** con **410** para inválido/expirado/reusado; su contrato no cambia (el `peek` es aditivo). La idempotencia (NFR-34) la garantiza el propio token de un solo uso (at-most-once por precondición de estado, ADR-0002), sin operationId aparte.
...
- **Criterios de aceptación:**
  ...
  - [ ] **Verificación · peek + ramificación por sesión (A5.2b, KER-63):** `POST /auth/email-verification/peek` con token válido devuelve `{ email }` (destino) **sin consumir** el token; inválido/expirado/reusado → 410 (sin distinguir). La pantalla de verificación usa el peek para ramificar: **sin sesión** → confirm + auto-login + feedback; **misma cuenta** → confirm sin pedir credenciales + feedback; **otra cuenta** → **no** confirm, cierra sesión y va a login con el email destino prefilleado + aviso (y `returnUrl` que retoma la verificación tras loguearse), **sin cambio de cuenta en silencio**.
  ...
  - [ ] **Emails de marca (KER-55, común a A4/A5 y a UC-03):** los tres emails transaccionales (invitación, reset, verificación) salen con la **plantilla HTML de marca** de Keru (logo con alt, paleta del brand book, CTA destacado, footer con identidad + motivo, aviso de "no respondas") y son **multipart** (parte HTML con estilos inline y layout de tablas + parte texto plano de respaldo) con el **mismo link/token en ambas partes**; el branding es **mejor esfuerzo** — no bloquea ni cambia la firma del flujo que envía el email. El logo se sirve desde el **CDN de marca por ambiente** (KER-70): `cdn.dev.keru.ar` en dev y `cdn.keru.ar` en prod (`EMAIL_LOGO_URL`), el **mismo host** que las fotos de paciente (`S3_PUBLIC_URL`) — emails y fotos cargan del mismo dominio por ambiente. Detalle de implementación en el brand book §9.
...
- **Flujos alternativos / excepciones:**
  - A1. **Cierre por vencimiento — reloj del ciclo de vida (NFR-14, timer-driven, KER-58):** la finalización de una asignación **nunca queda esperando a un actor**. El barrido de vencidos (scheduler cada 30 min o endpoint de ops) pasa a `historical` toda asignación `active` cuya ventana terminó (`periodEnd < now`) **y** cierra su contratación asociada (UC-09 A5). El barrido es idempotente y multi-instancia-safe (patrón claim `UPDATE...RETURNING`: cada fila la transiciona una sola instancia). El vencimiento **no** pisa un cierre previo por cancelación/no-show (esas asignaciones ya son `historical` y su request ya es `completed` con razón terminal).
...
- **Criterios de aceptación:**
  ...
  - [ ] Una asignación cuya ventana venció pasa a `historical` **sin intervención manual** (A1, NFR-14), en el mismo barrido que cierra su contratación (UC-09 A5).
...
- **Flujo principal:**
  ...
  6. Desde el círculo, un vínculo **`consent-holder`** puede **cambiar el rol** de otro miembro ya vinculado —promover un `viewer` a `manager`/`consent-holder`, o degradar— con **confirmación** en la UI; el cambio queda **auditado**. La **titularidad** nunca queda vacía: para ceder la titularidad se promueve antes a otro miembro a `consent-holder` y recién entonces puede degradarse el anterior (§2.4: el rol del vínculo gobierna qué puede hacer cada cuenta *sobre este paciente*).
- **Flujos alternativos / excepciones:**
  ...
  - A3. **Cambiar el rol de un miembro del círculo (solo `consent-holder`):** un vínculo `consent-holder` re-asigna el rol de otro miembro **ya vinculado** (a `consent-holder` / `manager` / `viewer`), vía `PATCH /patients/:id/links/:accountId {role}`. La autorización la decide el **`PermissionEngine`** (`hasLinkRole([consent-holder])`, §3.7), nunca inline: un `manager` o `viewer` que lo intente recibe **403**; si el objetivo no está vinculado al paciente, **404**; un `role` fuera del enum, **400**. Poner el mismo rol que ya tiene es un **no-op** idempotente (naturalmente idempotente, sin `operationId` — NFR-34/ADR-0002). Solo **Membership** escribe el vínculo (§3.3). La acción queda **auditada** (`membership.circle.link-role-changed`: actor, objetivo, rol anterior→nuevo, cuándo — §2.3).
  - A4. **Protección del último `consent-holder` (política de titularidad):** un cambio que dejaría al paciente con **cero** `consent-holder` —degradar al **único** titular a `manager`/`viewer`— se **rechaza** con **409 `LAST_CONSENT_HOLDER`**. La titularidad se **transfiere**, no se pierde: primero se promueve a otro miembro a `consent-holder` y después el titular anterior puede degradarse. Se admiten **múltiples** `consent-holder` (co-titulares); un paciente **nunca** puede quedar sin ninguno.
...
- **Criterios de aceptación:**
  ...
  - [ ] La **lista de perfiles** (y todo lugar que pinte la identidad del paciente: selector de perfil, selección de paciente al contratar) muestra la **foto** del paciente cuando tiene una cargada, con **fallback a las iniciales** cuando no. El listado del backend (`GET /patients`) expone `photoUrl` (opcional) por paciente, consistente con la ficha (`GET /patients/:id`) — KER-65.
  ...
  - [ ] Solo un vínculo **`consent-holder`** puede **cambiar el rol** de otro miembro del círculo (`PATCH /patients/:id/links/:accountId`), autorizado por el **`PermissionEngine`** (no inline): un `manager`/`viewer` recibe **403**, un objetivo no vinculado **404** y un rol inválido **400**. Roles válidos: `consent-holder` / `manager` / `viewer`. El cambio queda **auditado** (actor, objetivo, rol anterior→nuevo, cuándo).
  - [ ] **Nunca 0 `consent-holder` (titularidad):** degradar al **único** `consent-holder` se rechaza con **409 `LAST_CONSENT_HOLDER`**; para ceder la titularidad se promueve antes a otro miembro. Se admiten co-titulares (varios `consent-holder`), pero el paciente jamás queda sin ninguno.
  - [ ] La webapp del círculo ofrece el control de **cambio de rol** solo al `consent-holder` (con confirmación, reutilizando `kr-menu`/`kr-modal`); tras confirmar, el círculo refleja el nuevo rol sin recargar y muestra feedback de éxito. La **edición de ficha/avatar** está disponible para `consent-holder`/`manager` y un `viewer` ve la ficha en **solo lectura** con indicación clara (sin botón muerto).
...
- **Criterios de aceptación:**
  ...
  - [ ] **Insignias por-certificación en la card (KER-82):** la card del listado muestra también las certificaciones **aprobadas** del cuidador (ícono del catálogo + etiqueta), en un tratamiento **compacto** (fila de íconos con tooltip/`aria-label`, overflow `+N` si son muchas) que no rompe el layout con muchas ni con cero. Contrato: `CaregiverCardDto` expone `certifications` = `publicCertifications(...)` (**solo aprobadas**, sin la key privada del documento); el detalle (UC-07) ya las mostraba.
  - [ ] **Insignia unificada (KER-87):** especialidades, verificaciones (identidad/antecedentes) y certificaciones se muestran con el **mismo** componente y estilo tipográfico (`kr-insignias`, que consolida el `kr-badge` de especialidad + los badges de verificación + las insignias por-cert de KER-82; reusa `kr-cert-icon` de KER-77). Las **verificadas** (certificación aprobada, identidad, antecedentes) resaltan en **dorado**; las especialidades quedan neutrales. Solo se muestran las que el cuidador tiene (sin filas vacías). Todo con datos **ya expuestos** en el DTO (`specialties`, `certifications` con `verified`, `badges`) — sin cambios de contrato.
...
- **Criterios de aceptación:**
  ...
  - [ ] **Insignia unificada + verificada dorada (KER-87):** especialidades y certificaciones usan el mismo componente/estilo (`kr-insignias`) que la card (UC-06) y el admin (UC-19); las certificaciones **aprobadas** (y las verificaciones de identidad/antecedentes) resaltan en **dorado** con su `aria-label` ("Certificación verificada: …"). La ficha conserva institución y año por certificación.
...
- **Flujos alternativos / excepciones:**
  ...
  - A5. **Ciclo de vida temporal del servicio — timer-driven (NFR-14, KER-58).** El servicio transiciona **solo** por el reloj del barrido de vencidos (scheduler cada 30 min o endpoint de ops), sin esperar a ningún actor; dos transiciones idempotentes y multi-instancia-safe (patrón claim `UPDATE...RETURNING`):
    - **`accepted → in-progress` ("en curso") al entrar en ventana:** cuando el servicio aceptado alcanza su `startDate` y aún no venció (`startDate ≤ now < endDate`). Estado real y observable: el webapp lo muestra con badge "En curso" (success) tanto para el cuidador (Mis servicios) como para el solicitante (Mis contrataciones).
    - **`accepted | in-progress → completed` al pasar `endDate`:** cuando la ventana terminó (`endDate < now`), el barrido cierra la contratación con **razón terminal `completed`** — es un cierre normal por cumplimiento del período, no una cancelación — en el **mismo barrido** que historifica la asignación (UC-05 A1). Al ser `completed`, el servicio **sale de "activos"**, aparece en **"Finalizados"** y **habilita la calificación** (UC-17/21, NFR-20) sin intervención manual. El honor-mark de pago (OQ-1) sigue siendo opcional y posterior, sin condicionar el cierre.
    - **Precondición de no-clobber:** ambas transiciones exigen por SQL que el estado sea el de partida (`accepted`, o `accepted|in-progress`), así **nunca pisan** un cierre previo por cancelación/no-show (A3/A4) — ese request ya es `completed` con su razón terminal — ni la separación completado/pagado (KER-31). El cierre manual por el solicitante (`/complete`) sigue disponible y es idempotente frente al barrido (misma precondición de estado).
...
- **Criterios de aceptación:**
  ...
  - [ ] La solicitud tiene un ciclo de vida con estados: **pendiente → aceptada / rechazada / cancelada / vencida → en curso → completada**, que habilita los flujos posteriores de asignación, métricas y reseñas. Las transiciones **aceptada → en curso** (al entrar en ventana) y **en curso → completada** (al pasar `endDate`) son **timer-driven** (A5, NFR-14): las ejecuta el barrido, no un actor. *(Si se incluye el módulo de pagos, se insertará un estado "pagada" entre la aceptación y el inicio del servicio.)*
...
- **Flujo principal:**
  ...
  8. **Campana paginada (KER-86, perf).** El listado del centro de notificaciones se sirve **paginado por cursor descendente por `createdAt`** (desempate por `id`, orden DESC estable): la primera página trae un tamaño acotado (default 20, máx 100) y el panel pide las siguientes bajo demanda ("mostrar más"), appendeando sin repetir ni saltear. Un usuario con miles de notificaciones ya no recibe todo en una respuesta. El **contador de no leídas** sigue siendo un conteo aparte (badge), independiente de la paginación, y el **refetch-por-push** (paso 5) re-sincroniza desde la primera página.
- **Flujos alternativos / excepciones:**
...
- **Criterios de aceptación:**
  ...
  - [ ] **Campana paginada (KER-86):** el listado se pagina por cursor descendente por `createdAt` (default sano, no devuelve todo); el panel appendea las páginas siguientes con "mostrar más" y el contador de no leídas es un conteo aparte, intacto.
...
- **Descripción:** Proceso interno/manual con dos resultados: (1) **aprobar la cuenta nueva del cuidador**, requisito para que sea visible en el marketplace; (2) verificar credenciales, identidad y antecedentes, otorgando las insignias correspondientes. **Verificación de certificaciones por-certificación (KER-52):** en lugar de un único toggle grueso, el admin aprueba/rechaza **cada certificación individual** descargando su **documento privado** adjunto; cada certificación tiene su **propia insignia** (del catálogo, UC-02) que solo se muestra al público cuando esa certificación está **aprobada**.
...
- **Flujo principal:**
  ...
  2. Revisa la documentación del cuidador. **Por cada certificación (KER-52)** descarga su **documento privado** adjunto (PDF/imagen) por un endpoint **autorizado solo admin** (otros roles → 403); cada descarga queda **auditada**. El documento **nunca** tiene URL pública.
  3. **Aprueba la cuenta**: el perfil pasa a publicado y visible en el marketplace (UC-06).
  4. **Aprueba o rechaza cada certificación individual** (con motivo si la rechaza): una certificación aprobada queda **verificada** y su **insignia** (la del catálogo) pasa a mostrarse en el marketplace/ficha; una rechazada o pendiente **no se muestra**. Además marca los niveles independientes **identidad validada** y **antecedentes**.
  5. El sistema actualiza las insignias del perfil, visibles en listado y perfil (UC-06/07). La **insignia agregada `certificaciones`** (compat UC-06/07) queda **derivada**: es verdadera si el cuidador tiene **≥1 certificación aprobada** — no un toggle manual. Los niveles **identidad** y **antecedentes** siguen siendo toggles independientes.
- **Flujos alternativos / excepciones:**
  - A1. El administrador rechaza la cuenta: el perfil no se publica y se informa el motivo al cuidador.
  - A2. **Rechazo de una certificación (KER-52):** el admin rechaza una certificación puntual con motivo; esa certificación queda **rechazada** (no verificada, oculta al público) sin afectar a las demás ni al estado de la cuenta. La certificación rechazada es **inmutable** (§7); para reintentar, el cuidador **agrega una nueva** (UC-02 A4).
- **Postcondiciones:** Cuenta aprobada (o rechazada); certificaciones aprobadas/rechazadas una a una e insignias (por-cert + agregada derivada) actualizadas.
- **Criterios de aceptación:**
  - [ ] **Ningún cuidador aparece en el marketplace sin aprobación previa del administrador.**
  - [ ] Los tres niveles de verificación son independientes entre sí. La insignia **`certificaciones`** es **derivada** (≥1 certificación aprobada); **identidad** y **antecedentes** son toggles.
  ...
  - [ ] **Insignias por-certificación en el listado de gestión (KER-82):** el listado `GET /admin/caregivers` (`/admin/caregivers`) muestra, además de las insignias de verificación, las **insignias por-certificación** del cuidador. A diferencia del marketplace (solo aprobadas), la vista de gestión muestra **todas con su estado** (`ownerCertifications` → pendiente/verificada/rechazada), tiñendo cada insignia según el estado, porque el admin necesita ver qué falta revisar. El dato ya viaja en el DTO del listado (`CaregiverResponseDto.certifications`); era solo un gap de render. Tratamiento compacto (fila + overflow `+N`), cada insignia con `aria-label`/`title` (etiqueta + estado), sin depender solo del color/ícono (AA).
  - [ ] **Insignia unificada admin↔marketplace (KER-87):** el listado y el detalle admin usan el **mismo** componente/estilo de insignia (`kr-insignias`) que el marketplace (UC-06/07): especialidades neutrales; certificaciones **aprobadas** e identidad/antecedentes en **dorado** ("verificada"); pendientes/rechazadas por estado (warning/danger). El detalle admin conserva su tabla funcional de certificaciones (documento privado, aprobar/rechazar) con el estado "Verificada" también en dorado. Consolida los chips dispersos previos (SVGs inline de verificación, `kr-badge` de especialidad) en una sola pieza. Sin cambios de contrato (los datos ya se exponen).
...
| **Certificación** | **tipo del catálogo finito** (`catalogKey`, no texto libre — KER-52), institución, año, **documento privado adjunto** (`documentKey`, key de almacenamiento no público; NO URL pública), **estado por-cert** (pendiente / aprobada / rechazada) + quién/cuándo revisó + motivo de rechazo. Nace **pendiente y oculta**; inmutable una vez aprobada/rechazada (§7) | pertenece a Cuidador; su tipo referencia una entrada del **CatálogoCertificación** |
| **CatálogoCertificación** *(KER-52 / KER-77)* | catálogo **finito seedeado por migración** (patrón `range_version`): `key`, nombre visible, **insignia**: `iconKey` (ícono SVG diseñado del set Lucide, bundleado local en la webapp — KER-77) + `badgeIcon` (emoji, fallback text-only) | referenciado por Certificación (`catalogKey`) |
| **InsigniaVerificación** | nivel: **certificaciones verificadas (agregada, derivada: ≥1 certificación aprobada — KER-52)** / identidad validada / antecedentes. Además, **cada certificación aprobada muestra su insignia propia** del catálogo | pertenece a Cuidador |
